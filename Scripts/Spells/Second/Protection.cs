...
namespace Server.Spells.Second
{
	public class ProtectionSpell : MagerySpell
	{
		...
		public static void Toggle( Mobile caster, Mobile target )
		{
			...
			object[] mods;
			m_Table.TryGetValue( target, out mods );
			...
			if ( mods == null )
			{
				...
				m_Table[target] = mods;
				m_Registry[target] = 100.0;

				target.AddResistanceMod( mods[0] as ResistanceMod );
				target.AddSkillMod( mods[1] as SkillMod );
		...
		public static void EndProtection( Mobile m )
		{
			...
			{
				object[] mods;
				m_Table.TryGetValue( m, out mods );
