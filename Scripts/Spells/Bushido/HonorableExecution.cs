...
namespace Server.Spells.Bushido
{
	public class HonorableExecution : SamuraiMove
	{
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			HonorableExecutionInfo info;
			m_Table.TryGetValue( defender, out info );
			...
			if ( !defender.Alive )
			{
				...
				info.m_Timer = Timer.DelayCall<HonorableExecutionInfo>( TimeSpan.FromSeconds( 20.0 ), new TimerStateCallback<HonorableExecutionInfo>( EndEffect ), info );
			...
			else
			{
				...
				info.m_Timer = Timer.DelayCall<HonorableExecutionInfo>( TimeSpan.FromSeconds( 7.0 ), new TimerStateCallback<HonorableExecutionInfo>( EndEffect ), info );
		...
		{
			HonorableExecutionInfo info;
			m_Table.TryGetValue( target, out info );
