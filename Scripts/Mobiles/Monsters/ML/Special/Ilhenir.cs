...
namespace Server.Mobiles
{
	...
	public class Ilhenir : BaseChampion
	{
		...
		public virtual void CacophonicAttack( Mobile to )
		{
			...
			if( to.Alive && to.Player && m_Table[ to ] == null )
			{
				...
				m_Table[to] = Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 30 ), new TimerStateCallback<Mobile>( EndCacophonic_Callback ), to );
			}
		}

		private void EndCacophonic_Callback( Mobile to )
		{
			CacophonicEnd( to );
		}
	...
	public class StainedOoze : Item
	{
		...
		public override bool OnMoveOver(Mobile m)
		{
			if( m != null && !m.Deleted )
			{
				...
				if( m.Player || ( m is BaseCreature && ( (BaseCreature)m ).Controlled ) )
					m_Table[m] = Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 1 ), TimeSpan.FromSeconds( 1 ), new TimerStateCallback<Mobile>( Damage_Callback ), m );
			}
		...
		private void Damage_Callback( Mobile m )
		{
			Damage( m );
		}
		...
		{
			return ( !m.Deleted && m.Alive && m.AccessLevel == AccessLevel.Player && m.Map != null && m.Map != Map.Internal );
		}
