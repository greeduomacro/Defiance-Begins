...
namespace Server.Mobiles
{
	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		#region Scroll of Alacrity
		private DateTime m_AcceleratedEnd;

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime AcceleratedEnd
		{
			get { return m_AcceleratedEnd; }
			set { m_AcceleratedEnd = value; }
		}
			...
			if ( AccountHandler.LockdownLevel > AccessLevel.Player )
			{
				...
				if ( acct == null || !acct.HasAccess( from.NetState ) )
				{
					...
					if ( from.NetState != null )
						Timer.DelayCall<NetState>( TimeSpan.FromSeconds( 1.0 ), new TimerStateCallback<NetState>( Disconnect ), from.NetState );
				}
		...
		private void ValidateEquipment_Sandbox()
		{
			try
			{
				...
				#region Ethics
				Ethics.Player playerEPL = m_EthicPlayer;
				Ethics.Ethic ethic = m_EthicPlayer != null ? m_EthicPlayer.Ethic : null;
				#endregion
				...
				for ( int i = items.Count - 1; i >= 0; --i )
				{
					...
					if ( ethicsItem != null && ( ethicsItem.Ethic != ethic || playerEPL.HasFallen ) )
					{
		...
		private static void EventSink_Connected( ConnectedEventArgs e )
		{
			...
			Timer.DelayCall<Mobile>( TimeSpan.Zero, new TimerStateCallback<Mobile>( ClearSpecialMovesCallback ), e.Mobile );
		}

		private static void ClearSpecialMovesCallback( Mobile from )
		{
...
/*				List<AggressorInfo> list = target.Aggressors; //the people that target is being attacked by
				...
				for ( int i = 0; i < list.Count; i++ )
				{
					...
					if ( attacker != this && attacker != target && !aggressed.Contains( attacker ) && ( targetGuild == null || targetGuild != attacker.Guild ) && ( targetFaction == null || targetFaction != attackerFaction ) )
						aggressed.Add( attacker );
				}
*/
				//We care about the people that our target have provoked!

				List<AggressorInfo> list = target.Aggressed; //the people that target is attacking
				...
				for ( int i = 0; i < list.Count; i++ )
				{
					...
					if ( info.Provoked && defender != this && defender != target /*&& !aggressed.Contains( defender )*/ && ( targetGuild == null || targetGuild != defender.Guild ) && ( targetFaction == null || targetFaction != defenderFaction ) )
						aggressed.Add( defender );
				}

				int totalcount = aggressed.Count;

				// I am already enemies to these poeple, so they don't count for the message
				for ( int i = 0; i < this.Aggressed.Count; i++ )
					if ( aggressed.Contains( this.Aggressed[i].Defender ) )
						totalcount--;
...
*/
		...
		private static readonly string[] m_StaffTitles = new string[]
			{
				String.Empty,
				"Trial Counselor",
				"Counselor",
		...
		public string GetStaffTitle()
		{
			...
			else if ( AccessLevel > AccessLevel.Player )
			{
				switch ( AccessLevel )
				{
					case AccessLevel.Lead: return "Lead";
					case AccessLevel.Counselor: return m_StaffTitles[2];
					case AccessLevel.Seer: return m_StaffTitles[5];
					case AccessLevel.GameMaster: return m_StaffTitles[8];
					case AccessLevel.Administrator: return m_StaffTitles[11];
					case AccessLevel.Developer: return m_StaffTitles[14];
					case AccessLevel.Owner: return m_StaffTitles[16];
				}
			}
		...
		{
			get{ if ( String.IsNullOrEmpty( m_StaffTitle ) ) return GetStaffTitle(); else return m_StaffTitle; }
		...
		public PlayerMobile()
		{
			...
			m_SkillGainMods = new List<SkillGainMod>();
			...
			if ( from != null && Talisman is BaseTalisman )
			{
				...
				if ( talisman.Protection != null && talisman.Protection.Type != null )
				{
					...
					if ( type == from.GetType() )
						bonus -= (int)( ( (double) talisman.Protection.Amount ) / 100 );
				}
			}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 29:
				{
					...
					//Already created during serial constructor

					for ( int i = 0; i < skillgaincount; i++ )
						m_SkillGainMods.Add( new SkillGainMod( this, reader ) );
...
//						Timer.DelayCall( TimeSpan.Zero, new TimerCallback( RevertHair ) );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			int skillgaincount = 0;

			for ( int i = 0; i < m_SkillGainMods.Count; i++ )
				if ( m_SkillGainMods[i].Duration > TimeSpan.Zero && !m_SkillGainMods[i].HasElapsed() )
					skillgaincount++;
			...
			for ( int i = 0; i < m_SkillGainMods.Count; i++ )
				if ( m_SkillGainMods[i].Duration > TimeSpan.Zero && !m_SkillGainMods[i].HasElapsed() )
					m_SkillGainMods[i].Serialize( writer );
...
*/
		...
		private DateTime m_HonorTime;

		public DateTime HonorTime{ get{ return m_HonorTime; } set{ m_HonorTime = value; } }
		...
		public override string ApplyNameSuffix( string suffix )
		{
			...
			{
				if ( suffix.Length == 0 )
					suffix = m_EthicPlayer.Title();
				else
					suffix = String.Concat( suffix, " ", m_EthicPlayer.Title() );
			}
			#endregion
/*
...
*/
		...
		private List<SkillGainMod> m_SkillGainMods;

		public List<SkillGainMod> SkillGainMods{ get{ return m_SkillGainMods; } }

		public bool RemoveSkillGainMod( string name )
		{
			for( int i = 0; i < m_SkillGainMods.Count; ++i )
			{
				SkillGainMod check = m_SkillGainMods[i];

				if( check.Name == name )
				{
					SendSkillGainNormalMessage( check.Skill );
					m_SkillGainMods.RemoveAt( i );
					return true;
				}
			}
		...
		public void AddSkillGainMod( string name, SkillName skill, double bonus, TimeSpan duration )
		{
			...
			m_SkillGainMods.Add( new SkillGainMod( this, name, skill, bonus, duration ) );
		}
		...
		{
			SendMessage( "A magical force has accelerated your skill gain{0}{1}.", (skill == (SkillName)(-1)) ? String.Empty : " for " + Skills[skill].Name.ToLower(), duration > TimeSpan.Zero ? " temporarily" : String.Empty );
		}

		private void SendSkillGainNormalMessage( SkillName skill )
		{
			SendMessage( "Your skill gain rate{0} has return to normal.", (skill == (SkillName)(-1)) ? String.Empty : " for " + Skills[skill].Name.ToLower() );
		}
