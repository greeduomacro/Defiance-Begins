		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version < 1 || newVendorSystemActivated )
			{
				if ( version < 1 )
				{
					...
					Timer.DelayCall<bool>( TimeSpan.Zero, new TimerStateCallback<bool>( UpgradeFromVersion0 ), newVendorSystemActivated );
				}
				else
				{
		...
		public override bool CheckNonlocalDrop( Mobile from, Item item, Item target )
		{
			if ( IsOwner( from ) )
			{
				if ( GetVendorItem( item ) == null )
				{
					...
					Timer.DelayCall<NonLocalDropInfo>( TimeSpan.Zero, new TimerStateCallback<NonLocalDropInfo>( NonLocalDropCallback ), new NonLocalDropInfo( from, item ) );
				}
