...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
		...
		{
			if ( !TrickorTreatPersistence.TrickTreat( this, from ) )
			{
				Container theirPack = Backpack;

				double badKarmaChance = 0.5 - ((double)from.Karma / 8570);

				if ( theirPack != null )
				{
					if ( from.Karma < 0 && badKarmaChance > Utility.RandomDouble() )
						PublicOverheadMessage( MessageType.Regular, SpeechHue, 500406 ); // Thou dost not look trustworthy... no gold for thee today!
					else if ( from.CheckTargetSkill( SkillName.Begging, this, 0.0, 100.0 ) )
					{
						int toConsume = theirPack.GetAmount( typeof( Gold ) ) / 10;
						int max = Math.Max( Math.Min( 10 + (from.Fame / 2500), 14 ), 10 );
						...
						if ( toConsume > 0 )
						{
							...
							if ( consumed > 0 )
							{
								...
								if ( from.Karma > -3000 )
								{
									int toLose = from.Karma + 3000;

									if ( toLose > 40 )
										toLose = 40;

									Titles.AwardKarma( from, -toLose, true );
								}
							}
							else
