...
namespace Server.Mobiles
{
	public class AmazonGuardAI : BaseAI
	{
		...
		public override bool DoActionCombat()
		{
			...
			else if ( DateTime.Now >= m_NextCastTime && !m_Mobile.InRange( combatant, 8 ) && m_Mobile.InLOS( combatant ) && !combatant.Map.CanSpawnMobile( combatant.X, combatant.Y, combatant.Z ) && Utility.Random( 4 ) == 0 )
			{
				TeleportTo( combatant );
			}
			else if ( !m_Mobile.InRange( combatant, 3 ) )
			{
				if ( DateTime.Now >= m_NextCastTime && !m_Mobile.Move( m_Mobile.GetDirectionTo( combatant ) | Direction.Running ) && m_Mobile.InLOS( combatant ) && !combatant.Map.CanSpawnMobile( combatant.X, combatant.Y, combatant.Z ) )
					TeleportTo( combatant );
			}
