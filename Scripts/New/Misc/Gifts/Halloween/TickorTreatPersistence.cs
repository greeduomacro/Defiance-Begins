...
namespace Server.Misc
{
	public class TrickorTreatPersistence : Item
	{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			if ( BaseHalloweenGiftGiver.IsHalloween( DateTime.Now ) )
			{
				writer.Write( true );
		...
		private class CandyTimer : Timer
		{
				...
				: base( TimeSpan.FromSeconds( 15.0 ), TimeSpan.FromSeconds( 15.0 ) )
			{
				m_From = from;
				m_ToothacheLevel = 1;
				m_NextAcheCheck = DateTime.Now + TimeSpan.FromMinutes( 3.0 + ( Utility.RandomDouble() * 1.5 ) );
			...
			protected override void OnTick()
			{
				...
				if ( DateTime.Now >= m_NextAcheCheck )
				{
