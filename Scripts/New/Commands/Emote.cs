...
namespace Server.Commands
{
	public class Emote
	{
		...
		private static List<Mobile> m_Table = new List<Mobile>();
		...
		public static void DoEmote( Mobile from, int sound )
		{
			if ( !m_Table.Contains( from ) )
			{
				if ( from.Squelched || !from.Alive )
					from.SendMessage( "You cannot act out in your current state." );
				else
				{
					switch( sound )
					{
					...
					Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 4.0 ), new TimerStateCallback<Mobile>( ReleaseEmote_Callback ), from );
				}
			}
		}
			...
			switch ( type )
			{
				...
				case 1:
				{
					if ( index > 0 && index < 43 )
					{
						...
						from.SendGump( new EmoteGump( from, (index / 12) + 1 ) );
					}
					break;
				}
			}
	...
	{
		[Constructable]
		public Puke() : base( Utility.Random( 0xF3B, 2 ) )
		{
			Name = "puke";
			Hue = 0x557;
			Movable = false;
