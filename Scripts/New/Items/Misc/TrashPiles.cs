...
namespace Server.Items
{
	public class BaseTrash : BaseAddon
	{
		...
		private double m_ItemChance = 0.25;
		private double m_RareChance = 0.005;
		...
		public virtual void OnLoot(Mobile from)
		{
			...
			if (!FindLooter(from))
			{
				...
				if ( m_RareChance + useskills > Utility.RandomDouble() )// plus chance for a random prize
				{
				...
				else if ( ( m_RareChance + useskills ) / 10.0 > Utility.RandomDouble() )
				{
		...
		public void DefragLooters()
		{
			...
			for (int i = 0; i < Looters.Count; i++)
			{
				try
				{
					...
					if ( looter.Time + m_LootWait < DateTime.Now || looter.Looter == null )
						Looters.RemoveAt(i--);
				}
				catch (Exception e)
				{
