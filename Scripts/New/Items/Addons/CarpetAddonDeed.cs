...
namespace Server.Items
{
	public class CarpetAddonDeed : BaseAddonDeed
	{
		...
		public override string DefaultName{ get{ return "a deed to a carpet"; } }
		...
		private void BoundingBox_Callback( Mobile from, Map map, Point3D start, Point3D end, object state )
		{
			...
			if ( width < 2 || height < 2 )
				from.SendMessage( "The carpet must be at least 3 tiles in size." );
			else if ( width > 10 || height > 10 )
				from.SendMessage( "The carpet cannot be greater than 10 tiles in size." );
