...
namespace Server.Items
{
	public class UnholySkullTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object target )
		{
			...
			else if ( item.LootType != LootType.Regular || item.BlessedFor != null || (Mobile.InsuranceEnabled && item.Insured) ) // Check if its already newbied (blessed)
			...
			else
			{
				...
				item.Hue = 1175;
				item.Name = String.Format( "{0} blessed by an unholy skull", String.IsNullOrEmpty( item.Name ) ? item.ItemData.Name : item.Name );
	...
	public class UnholySkullDeed : Item
	{
		...
		public UnholySkullDeed() : base( 0x2251 )
		{
			Hue = 1175;
			Weight = 15.0;
			Name = "an unholy skull of blessing";
