...
namespace Server.Items
{
	public class UnblessTarget : Target // Create our targeting class (which we derive from the base target class)
	{
		...
		protected override void OnTarget( Mobile from, object target ) // Override the protected OnTarget() for our feature
		{
			...
			if ( item == null || item.RootParent != from || item.LootType != LootType.Blessed || item.BlessedFor != from || (Mobile.InsuranceEnabled && item.Insured) ) // Check if its already newbied (blessed)
	...
	public class UnblessDeed : Item // Create the item class which is derived from the base item class
	{
		...
		{
			Weight = 1.0;
			Name = "a clothing bless removal deed";
