...
namespace Server.Items
{
	...
	public class Moongate : Item
	{
		...
		public virtual void CheckGate( Mobile m, int range )
		{
			...
			if ( Core.ML && m.Hidden && m.AccessLevel == AccessLevel.Player )
			{
				m.RevealingAction();
		...
		public virtual void UseGate( Mobile m )
		{
			...
			//else if ( (m.Kills >= Mobile.MurderCount && m_TargetMap != Map.Felucca) || ( m_TargetMap == Map.Tokuno && (flags & ClientFlags.Tokuno) == 0 ) || ( m_TargetMap == Map.Malas && (flags & ClientFlags.Malas) == 0 ) || ( m_TargetMap == Map.Ilshenar && (flags & ClientFlags.Ilshenar) == 0 ) )
