...
namespace Server.Items
{
	public abstract class BaseConfusionBlastPotion : BasePotion
	{
		...
		public virtual void Explode( Mobile from, Point3D loc, Map map )
		{
			...
			Timer.DelayCall<IEntity>( TimeSpan.FromSeconds( 0.3 ), new TimerStateCallback<IEntity>( CircleEffect2 ), new Entity( Serial.Zero, loc, map ) );
		...
		{
			Geometry.Circle2D( ent.Location, ent.Map, Radius, new DoEffect_Callback( BlastEffect ), 90, 270 );
		}
		#endregion
		...
		public static void AddDelay( Mobile m )
		{
			...
			m_Delay[m] = Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 60 ), new TimerStateCallback<Mobile>( EndDelay ), m );
		}
		...
		private class ThrowTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				Effects.SendMovingEffect( from, p is Mobile ? (Mobile)p : to, 0xF0D, 7, 0, false, false, m_Potion.Hue, 0 );
				Timer.DelayCall<IEntity[]>( TimeSpan.FromSeconds( 1.0 ), new TimerStateCallback<IEntity[]>( m_Potion.Explode_Callback ), new IEntity[]{ from, to } );
			}
		}
	}
}