...
namespace Server.Items
{
	...
	public abstract class BaseInstrument : Item, ICraftable, ISlayer
	{
		...
		private static Dictionary<Mobile, Timer> m_LogoutCleanup = new Dictionary<Mobile, Timer>();
		...
		private static void OnLogout( LogoutEventArgs e )
		{
			...
			if ( m_Instruments.ContainsKey( from ) )
				m_LogoutCleanup[from] = Timer.DelayCall<Mobile>( TimeSpan.FromMinutes( 60.0 ), new TimerStateCallback<Mobile>( ExpireInstrumentPick ), from );
		}
		...
		{
			return m.CheckSkill( SkillName.Musicianship, 0.0, 120.0 );
