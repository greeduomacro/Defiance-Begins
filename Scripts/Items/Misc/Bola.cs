...
namespace Server.Items
{
	public class Bola : Item
	{
		...
		private static void ReleaseBolaLock( Mobile from )
		{
			from.EndAction( typeof( Bola ) );
		}

		private static void FinishThrow( Mobile[] states )
		{
			Mobile from = states[0];
			Mobile to = states[1];

			//What the fuck is this? Lets add new random bolas to the game! :\
			//if ( Core.AOS )
			...
			Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 2.0 ), new TimerStateCallback<Mobile>( ReleaseBolaLock ), from );
		}
			...
			protected override void OnTarget( Mobile from, object obj )
			{
				...
				if ( obj is Mobile )
				{
					...
					else if ( from.BeginAction( typeof( Bola ) ) )
					{
						...
						Timer.DelayCall<Mobile[]>( TimeSpan.FromSeconds( 0.5 ), new TimerStateCallback<Mobile[]>( FinishThrow ), new Mobile[]{ from, to } );
					}
					else
					{
						from.SendLocalizedMessage( 1049624 ); // You have to wait a few moments before you can use another bola!
