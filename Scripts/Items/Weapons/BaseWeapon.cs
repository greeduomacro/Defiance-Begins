...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, IEthicsItem
	{
...
/*
			...
			{ */
				...
				//Penalize people for having lower skill, up to 10% slower.

				double skill = m.Skills[GetUsedSkill( m, false )].Value / 100.0;
				double realSpeed = speed - ( (speed * 0.10) * ( 1.0 - skill ) );

				double v = (m.Stam + 100) * realSpeed;
		...
		public virtual bool CanSwing( Mobile attacker, Mobile defender )
		{
			...
			return !( attacker.Paralyzed || attacker.Frozen ) && ( sp == null || !sp.IsCasting || !sp.BlocksMovement ) && ( p == null || p.PeacedUntil <= DateTime.Now );
		}
		...
		public static bool CheckParry( Mobile defender )
		{
			...
			if ( shield != null )
			{
				double chance = (parry - bushidoNonRacial) / 500.0;	// As per OSI, no negitive effect from the Racial stuffs, ie, 120 parry and '0' bushido with humans
...
*/
			...
			if ( m_MaxHits > 0 && (acidblood || Utility.Random( Math.Max( attacker.Str / 4, 10 ) ) == 0) ) // Stratics says 50% chance, seems more like 4%..
			{
				if ( acidblood )
...
*/
		...
		public virtual int GetHitChanceBonus( Mobile attacker )
		{
			...
			if ( m_EthicState != null && m_EthicState.IsRunic && attackerState != null && attackerState != m_EthicState.Ethic ) //Assume that Ethic is never null for items which have ethical states
				bonus += 10;
			else
			{
...
//				damageBonus = 100;
		...
		public virtual double ScaleDamageOld( Mobile attacker, double damage, bool checkSkills )
		{
			...
			if ( m_EthicState != null && m_EthicState.IsRunic && attackerState != null && attackerState != m_EthicState.Ethic ) //Assume that Ethic is never null for items which have ethical states
				...
				bonus += (attacker.Skills[SkillName.Tactics].Value - 50.0) / 100.0;

			/* Compute strength modifier
			 * : 1% bonus for every 5 strength
			 */
			bonus += (attacker.Str / 5.0) / 100.0;
