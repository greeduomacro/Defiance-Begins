...
namespace Server.Gumps
{
	public class PropertiesGump : Gump
	{
			...
			switch ( info.ButtonID )
			{
				...
				default:
				{
					...
					if ( index >= 0 && index < m_List.Count )
					{
						...
						else if ( IsType( type, typeofAI ) )
							from.SendGump( new SetListOptionGump( prop, from, m_Object, m_Stack, m_Page, m_List, MobileAIRegistry.GetAINames(), MobileAIRegistry.GetAIValues() ) );
		...
		private static Type typeofAI = typeof( BaseAI );
