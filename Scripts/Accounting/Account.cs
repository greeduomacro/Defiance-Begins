...
namespace Server.Accounting
{
	public class Account : IAccount, IComparable, IComparable<Account>
	{
		...
		private string m_Username, m_PlainPassword, m_CryptPassword, m_NewCryptPassword, m_SaltedCryptPassword;
		...
		/// </summary>
		public string SecurityQuestion{ get { return m_SecurityQuestion; } }

		/// <summary>
		/// Account's security answer for password recovery.
		/// </summary>
		public string SecurityAnswer{ get { return m_SecurityAnswer; } }
		...
		private static readonly string HashPrefix = "CAS2011!";
		private static readonly string HashSuffix = "TRAILENDER";
		...
		{
			m_SecurityQuestion = question;
			m_SecurityAnswer = HashSHA1( SaltPhrase( m_Username.ToLower() + plainAnswer.ToLower() ) );
		}

		public void SetSecurityQuestionSHA1( string question, string answer )
		{
			m_SecurityQuestion = question;
			m_SecurityAnswer = answer;
		}

		public bool CheckSecurityAnswer( string plainAnswer )
		{
			return m_SecurityAnswer == HashSHA1( SaltPhrase( m_Username.ToLower() + plainAnswer.ToLower() ) );
		}
		...
		public void SetPassword( string plainPassword )
		{
			switch ( AccountHandler.ProtectPasswords )
			{
				...
				default: //Salted Crypt
				{
					...
					m_CryptPassword = null;
					m_NewCryptPassword = null;
					m_SaltedCryptPassword = HashSHA1( SaltPhrase( m_Username.ToLower() + plainPassword ) );
		...
		public bool CheckPassword( string plainPassword )
		{
			...
			{
				ok = ( m_SaltedCryptPassword == HashSHA1( SaltPhrase( m_Username.ToLower() + plainPassword ) ) );
...
*/
		...
		public void LogIPAccess( IPAddress ipAddress, bool loggedOut )
		{
			...
			if ( !Directory.Exists( directory ) )
				...
				try
				{
					using ( StreamWriter op = new StreamWriter( Path.Combine( directory, String.Format( "{0}.log", m_Username.ToLower() ) ), true ) )
					{
						op.WriteLine( "{0} - {1}{2} - {3}", DateTime.Now, m_Username, loggedOut ? String.Empty : String.Format( "- {0}", ipAddress.ToString() ), loggedOut ? "off" : "on" );
					}
				}
				catch
				{
					Console.WriteLine( "Failed to write to file: {0}", Path.Combine( "Logs/Login", String.Format( "{0}.log", m_Username.ToLower() ) ) );
				}
		}
		...
		public void Save( XmlTextWriter xml )
		{
			...
			if ( m_SaltedCryptPassword != null )
			{
				xml.WriteStartElement( "saltedCryptPassword" );
				xml.WriteString( m_SaltedCryptPassword );
				xml.WriteEndElement();
			}
			...
			xml.WriteString( m_SecurityQuestion );
			xml.WriteEndElement();

			xml.WriteStartElement( "secanswer" );
			xml.WriteString( m_SecurityAnswer );
			xml.WriteEndElement();
