...
//		[Constructable]
		public Spawner( int amount, int minDelay, int maxDelay, int team, int homeRange, string spawnedName ) : base( 0x1f13 )
		{
...
//		[Constructable]
		...
		public Spawner( int amount, TimeSpan minDelay, TimeSpan maxDelay, int team, int homeRange, params string[] spawnedName ) : base( 0x1f13 )
		{
			InitSpawn( amount, minDelay, maxDelay, team, homeRange );
			for ( int i = 0;i < spawnedName.Length; i++ )
				AddEntry( spawnedName[i], 100, amount, false );
		}
...
*/
		...
		public void RemoveSpawns( int index ) //Entry
		{
			if ( index >= 0 && index < m_Entries.Count )
				RemoveSpawns( m_Entries[index] );
		}

		public void RemoveSpawns( SpawnerEntry entry )
		{
			for ( int i = entry.Spawned.Count-1; i >= 0; i-- )
			{
				ISpawnable e = entry.Spawned[i];
