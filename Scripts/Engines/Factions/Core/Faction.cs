...
namespace Server.Factions
{
	...
	public abstract class Faction : IComparable
	{
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register( "FactionElection", AccessLevel.Owner, new CommandEventHandler( FactionElection_OnCommand ) );
			CommandSystem.Register( "FactionCommander", AccessLevel.Owner, new CommandEventHandler( FactionCommander_OnCommand ) );
			CommandSystem.Register( "FactionItemReset", AccessLevel.Owner, new CommandEventHandler( FactionItemReset_OnCommand ) );
			CommandSystem.Register( "FactionReset", AccessLevel.Owner, new CommandEventHandler( FactionReset_OnCommand ) );
			CommandSystem.Register( "FactionTownReset", AccessLevel.Owner, new CommandEventHandler( FactionTownReset_OnCommand ) );
		}

		private static readonly TimeSpan MustKillThisOften = TimeSpan.FromDays( 90.0 );
		...
		public const int StabilityFactor = 20; // 20% greater than smallest faction
		public const int StabilityActivation = 250; // Stablity code goes into effect when largest faction has > 200 people
		...
		public static void HandleDeath( Mobile victim, Mobile killer )
		{
			...
			if ( pack != null )
			{
				...
				for ( int i = 0; i < sigils.Length; ++i )
				{
					...
					if ( killerState != null && killerPack != null )
					{
						...
						{
							sigil.ReturnHome();
							killer.SendMessage( "The sigil has gone back to its home location because sigils cannot be carried inside a house." );
						}
					}
					else
					{
			...
			if ( victim is BaseCreature && !(victim is BaseVendor) )
			{
				...
				if ( Faction.IsFactionFacet( bc.Map ) && victimFaction != null && killerState.Faction != victimFaction )
				{
...
/*
				#region Ethics
				if ( Faction.IsFactionFacet( bc.Map ) && bc.GetEthicAllegiance( killer ) == BaseCreature.Allegiance.Enemy )
				{
