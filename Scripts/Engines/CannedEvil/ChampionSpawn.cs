...
namespace Server.Engines.CannedEvil
{
	...
	public class ChampionSpawn : Item
	{
		...
		public int MaxLevel{ get{ return m_MaxLevel; } set{ m_MaxLevel = Math.Max( Math.Min( value, 18 ), 0 ); } }
		...
		public void SpawnChampion()
		{
			...
			{ Console.WriteLine( String.Format( "Exception creating champion {0}: {1}", m_Type, e ) ); }
		...
		public int Level1{ get{ return 4 + (m_MaxLevel - 16); } }
		public int Level2{ get{ return 9 + Math.Min( (m_MaxLevel - 16), 1 ); } }
		public int Level3{ get{ return 13 + Math.Min( (m_MaxLevel - 16), 1 ); } }
		public int Level4{ get{ return 17; } }
		public int Level5{ get{ return 18; } }
		...
		public virtual void AwardArtifact( Item artifact )
		{
			...
			if ( m_DamageEntries.Count > 0 )
			{
				...
				do
				{
					...
					{
						checkDamage += kvp.Value;

						if ( checkDamage > randomDamage )
						{
							if ( GiveArtifact( kvp.Key, artifact ) )
								artifactGiven = true;
							else
								validEntries.Remove( kvp.Key );
	...
	public class ChampionSpawnRegion : BaseRegion
	{
		...
		public override void OnEnter( Mobile m )
		{
			if ( m.Player && m.AccessLevel == AccessLevel.Player && !m_Spawn.Active )
			{
				...
				if ( ((PlayerMobile)m).Young && parent is BaseRegion && ((BaseRegion)parent).YoungProtected )
					m.SendMessage( "You decide against advancing to this part of the dungeon because of the dangers." );
				else if ( m_Spawn.ReadyToActivate )
					m_Spawn.Start();
				else if ( m_Spawn.ProximitySpawn )
				{
					...
					for ( int i = 0; i < players.Count; i++ )
						if ( players[i].NetState != null && !addresses.Contains( players[i].NetState.Address ) && !((PlayerMobile)players[i]).Young )
