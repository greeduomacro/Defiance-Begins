...
namespace Server.Misc
{
	...
	public class NameVerification
	{
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register( "ValidatePlayerName", AccessLevel.Administrator, new CommandEventHandler( ValidatePlayerName_OnCommand ) );
		}
		...
		public static void ValidateName_OnCommand( CommandEventArgs e )
		{
			if ( Validate( e.ArgString, 2, 16, true, false, true, 1, SpaceDashPeriodQuote ) != NameResultMessage.Allowed )
				e.Mobile.SendMessage( 0x59, "That non-player name is considered valid." );
			else
				e.Mobile.SendMessage( 0x22, "That non-player name is considered invalid." );
		}
		...
		{
			if ( ValidatePlayerName( e.ArgString, 2, 16, true, false, true, 1, SpaceDashPeriodQuote ) != NameResultMessage.Allowed )
				e.Mobile.SendMessage( 0x59, "That player name is considered valid." );
			else
				e.Mobile.SendMessage( 0x22, "That player name is considered invalid." );
		}
/*
...
*/
		...
		{
			return Validate( name, minLength, maxLength, allowLetters, allowDigits, noExceptionsAtStart, maxExceptions, exceptions, m_DisallowedWords, m_StartDisallowed, m_DisallowedAnywhere );
		}

		public static NameResultMessage Validate( string name, int minLength, int maxLength, bool allowLetters, bool allowDigits, bool noExceptionsAtStart, int maxExceptions, char[] exceptions, string[] disallowedWords, string[] startDisallowed, string[] disallowedAnywhere )
		{
			...
			for ( int i = 0; i < disallowedWords.Length; ++i )
			{
				int indexOf = name.IndexOf( disallowedWords[i] );
				...
				bool badSuffix = ( (indexOf + disallowedWords[i].Length) >= name.Length );

				for ( int j = 0; !badSuffix && j < exceptions.Length; ++j )
					badSuffix = ( name[indexOf + disallowedWords[i].Length] == exceptions[j] );
			...
			for ( int i = 0; i < disallowedAnywhere.Length; i++ )
				if ( name.IndexOf( disallowedAnywhere[i] ) > -1 )
					return NameResultMessage.NotAllowed;
		...
		{
			return ValidatePlayerName( name, minLength, maxLength, allowLetters, allowDigits, noExceptionsAtStart, maxExceptions, exceptions, m_DisallowedWords, m_StartDisallowed, m_DisallowedAnywhere );
		}

		public static NameResultMessage ValidatePlayerName( string name, int minLength, int maxLength, bool allowLetters, bool allowDigits, bool noExceptionsAtStart, int maxExceptions, char[] exceptions, string[] disallowedWords, string[] startDisallowed, string[] disallowedAnywhere )
		{
			NameResultMessage result = Validate( name, minLength, maxLength, allowLetters, allowDigits, noExceptionsAtStart, maxExceptions, exceptions, disallowedWords, startDisallowed, disallowedAnywhere );
			...
			if ( result == NameResultMessage.Allowed )
			{
				...
				foreach ( Mobile m in mobs )
					if ( m is PlayerMobile /*&& AccessLevelToggler.GetRawAccessLevel( (PlayerMobile)m ) == AccessLevel.Player*/ && !String.IsNullOrEmpty( m.RawName ) && m.RawName.Trim().ToLower() == lowername )
