...
namespace Server.Misc
{
	public class AutoSave : Timer
	{
		...
		private static readonly TimeSpan m_Warning = TestCenter.Enabled ? TimeSpan.Zero : TimeSpan.FromSeconds( 60.0 );
		...
		public static void Save( bool permitBackgroundWrite )
		{
			...
			{
				//Console.WriteLine( "World: Backing up files. Save delayed for 5 seconds." );
				Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), new TimerCallback( Save ) );
			}
			else if ( WorldBackup.BackupSuccess )
			{
				...
				Console.WriteLine( "World: Previous backup was not successful." );
		}
