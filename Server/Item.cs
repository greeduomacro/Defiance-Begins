...
*/
		...
		public virtual bool OnDroppedInto( Mobile from, Container target, Point3D p )
		{
			if( !from.OnDroppedItemInto( this, target, p ) )
				return false;
			else if( Nontransferable && from.Player && target != from.Backpack && ( QuestItem || target != from.BankBox ) && from.AccessLevel <= AccessLevel.GameMaster )
			{
				HandleInvalidTransfer( from );
				return false;
			}
		...
		{
			return ( m_LootType == LootType.Newbied && from.IsYoung() );
		}
